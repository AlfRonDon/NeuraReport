...
- [DONE - 2026-01-27] POST `/analytics/insights|trends|anomalies|correlations|whatif` - AI analytics endpoints
  - **Audit Findings**: API-key auth ✓. Issues: (1) ~~POST /activity uses bare params not Pydantic model~~ **FIXED**: Created `LogActivityRequest` Pydantic model with `action` (1-100 chars), `entity_type` (1-50 chars), optional `entity_id`/`entity_name`/`details` (2) ~~PUT /preferences bulk has no size validation~~ **FIXED**: Added `MAX_PREFERENCES_SIZE_BYTES = 50,000` with JSON body size check; returns 413 on exceeded (3) ~~POST /notifications uses raw dict~~ **FIXED**: Created `CreateNotificationRequest` Pydantic model with `title` (1-255), `message` (max 2000), `type` (info|success|warning|error pattern), optional `link`/`entityType`/`entityId` (4) ~~Bulk operations lack size limits~~ **FIXED**: Created `BulkTemplateRequest` (max 500 IDs) and `BulkJobRequest` (max 500 IDs) Pydantic models; all 5 bulk endpoints now use typed models (5) ~~DELETE /activity and /notifications are destructive with no confirmation~~ **FIXED**: Both `DELETE /activity` and `DELETE /notifications` now require `X-Confirm-Destructive: true` header; returns 400 if missing. Also sanitized bulk operation error messages — replaced `str(e)` with `type(e).__name__` across all 5 bulk endpoints (6) In-memory global search doesn't scale
...
- [DONE - 2026-01-27] POST `/charts/generate` - Generate chart config (1-1000 rows, chart_type, x_field, y_fields). Supports background mode.
  - **Audit Findings**: API-key auth ✓. Issues: (1) ~~No chart_type enum validation~~ **FIXED**: Added `VALID_CHART_TYPES` set (bar, line, scatter, pie, area, heatmap, histogram, box, radar, treemap) with `field_validator("chart_type")` on `ChartGenerateRequest` (2) ~~No x_field/y_fields cross-validation against data keys~~ **FIXED**: Added `model_validator` to cross-validate `x_field` and `y_fields` against actual data keys when data is present (3) ~~Background mode error messages may leak internals~~ **FIXED**: Sanitized error messages — replaced `str(exc)` with `type(exc).__name__` in background task error callbacks
...
- [DONE - 2026-01-27] GET `/connections/{id}/preview` - Preview table rows (limit 1-200, offset, table required)
  - **Audit Findings**: API-key auth ✓. Issues: (1) ~~NO error handling on ANY endpoint — bare exceptions bubble as 500~~ **FIXED**: Added `_handle_connection_error()` helper; all 7 endpoints wrapped in try/except with `(HTTPException, AppError)` propagation and generic 500 for unhandled exceptions (2) ~~DELETE doesn't verify existence~~ **FIXED**: DELETE now calls `svc.repo.get(connection_id)` before deletion; returns 404 if not found (3) Schema/preview use legacy imports (4) ~~`table` param has no SQL injection protection at API layer~~ **FIXED**: Added `_SQL_IDENTIFIER_RE` regex (`^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)*$`) validation on `table` query param in preview endpoint; rejects invalid identifiers with 400 (5) ~~`svc.repo.list()` bypasses service abstraction~~ **FIXED**: Added `list()` method to `ConnectionService`; route now calls `svc.list()` instead of bypassing to `svc.repo.list()`
...
- [DONE - 2026-01-27] GET/POST `/connectors/{type}/oauth/*` - OAuth authorize + callback
  - **CRITICAL AUDIT**: (1) ~~NO auth on ANY endpoint~~ **FIXED (Group A)**: `require_api_key` dependency added to router (2) ~~SQL injection risk on /query~~ **FIXED (Group C)**: Added `is_read_only_sql()` validation — blocks DDL/DML (DROP, ALTER, DELETE, INSERT, UPDATE, etc.); only SELECT/WITH allowed (3) ~~Credentials stored in plaintext in-memory dict~~ **FIXED**: Credentials now stored separately in `connector_credentials` namespace; main `connectors` record stripped of raw config; `_store_get_config()` accessor for credential retrieval (4) ~~OAuth tokens returned in response body unredacted~~ **FIXED**: Added `_redact_tokens()` — replaces token/secret values with `***`, adds `_redacted: True` flag (5) ~~redirect_uri not validated (open redirect)~~ **FIXED**: Added `_validate_redirect_uri()` using `is_safe_external_url()` — blocks localhost, private IPs, non-HTTP schemes (6) ~~Connector not disconnected on error (connection leak)~~ **FIXED**: Added `try/finally` with `connector.disconnect()` on schema and query endpoints (7) ~~Path ambiguity between {connector_type} and {connection_id}~~ **FIXED**: Added UUID regex pattern `^[0-9a-f-]{36}$` on all `connection_id` Path params to disambiguate from `{connector_type}` routes. Also sanitized all error messages — replaced `str(e)` with `type(e).__name__`
...
- [DONE - 2026-01-27] POST `/dashboards/analytics/*` - Insights, trends, anomalies, correlations — STUBS
  - **Audit Findings**: ~~NO auth on ANY endpoint~~ **FIXED (Group A)**: `require_api_key` dependency added to router. Issues: (1) ~~In-memory storage not persistent~~ **FIXED**: Migrated to StateStore persistence (2) Embed token is random UUID not JWT — unverifiable (3) ~~WidgetConfig.query field accepts raw SQL with no validation~~ **FIXED**: Added `field_validator("query")` calling `is_read_only_sql()` — blocks DDL/DML (DROP, ALTER, DELETE, INSERT, UPDATE, etc.); also added `max_length=10_000` on query field (4) ~~Analytics endpoints accept unlimited data payloads~~ **FIXED**: Added `MAX_ANALYTICS_ROWS = 10,000` cap on all 4 analytics endpoints (insights, trends, anomalies, correlations); returns 400 if exceeded. Also added `title: Field(min_length=1, max_length=255)` on WidgetConfig (5) ~~refresh_interval unbounded~~ **FIXED**: Changed to `Field(None, ge=5, le=86400)` — minimum 5 seconds, maximum 24 hours (6) Several endpoints return stub data
...
- [DONE - 2026-01-27] DELETE `/docqa/sessions/{id}/history` - Clear chat history. Verified at docqa.py:250. Discovered endpoint not in original audit.
  - **Audit Findings**: Auth ✓ (`require_api_key`). Issues: (1) ~~`limit` on history has no `ge`/`le` constraint~~ **FIXED (Group F)**: Changed to `limit: int = Query(50, ge=1, le=500)` (2) ~~`DocumentQAService()` instantiated per request via `get_service()` — no singleton caching~~ **FIXED**: Converted to singleton pattern using `global _docqa_service` with lazy initialization (3) ~~`regenerate_response` catches only `RuntimeError`~~ **FIXED (Group G)**: Added `except Exception` after `except RuntimeError` — logs and returns generic 500 (4) `list_sessions` has no pagination — unbounded results (5) Additional endpoint discovered: DELETE `/sessions/{id}/history`
...
- [DONE - 2026-01-27] DELETE `/enrichment/cache` - Clear cache. Verified at enrichment.py:214. Optional `source_id` query param (max 64 chars).
  - **Audit Findings**: Auth ✓ (`require_api_key`). **FIXED**: Error responses now return proper HTTP 404 status codes instead of 200 OK with `{"status": "error"}`. Issues: (1) ~~Built-in sources are hardcoded in route file (not service)~~ **FIXED**: Moved `AVAILABLE_SOURCES` catalog to `EnrichmentService.get_builtin_sources()` static method; route now calls service method (2) Discovered endpoints: GET `/source-types` (legacy), POST `/sources/create`
...
- [DONE - 2026-01-27] POST `/excel/reports/discover` - Discover available batches. Verified at excel.py:332. DiscoverPayload with full DI chain.
  - **Audit Findings**: Auth ✓ (`require_api_key`). Issues: (1) Original audit listed only 3 endpoints — actual file has 15+ endpoints (354 lines) (2) ~~File upload has no explicit size limit at route level~~ **FIXED**: Added `MAX_UPLOAD_SIZE_BYTES = 100 * 1024 * 1024` (100 MB); cumulative byte count checked during `_persist_upload`; returns 413 on size exceeded (3) ~~`_persist_upload` writes to temp file without size check~~ **FIXED**: Size check integrated into `_persist_upload` with proper temp file cleanup on rejection (4) ~~Background verify job creates UploadFile from disk — no cleanup on failure path~~ **FIXED**: Added temp file cleanup in both `HTTPException` and generic `Exception` handlers (5) ~~`keys/options` `limit` param defaults to 500 with no upper bound constraint~~ **FIXED**: Changed to `Query(500, ge=1, le=5000)` — bounded upper limit
...
- [DONE - 2026-01-27] POST `/export/distribution/webhook` - Generic webhook. Verified at export.py:224. **SSRF RISK**: `webhook_url` + arbitrary headers + configurable method.
  - **CRITICAL AUDIT**: (1) **FIXED** — Auth added (`require_api_key` dependency, was completely unauthenticated) (2) ~~**SSRF risk** on teams/webhook endpoints~~ **FIXED (Group B)**: Added `is_safe_external_url()` field validator to `WebhookDeliveryRequest.webhook_url` and `TeamsMessageRequest.webhook_url` in export schema — blocks localhost, private IPs (10.x, 172.16.x, 192.168.x), link-local (169.254.x), metadata endpoints, non-HTTP(S) schemes; resolves hostname and checks resolved IP (3) ~~`options: dict = None` on all format endpoints — accepts arbitrary untyped options~~ **FIXED**: Created typed `ExportOptions` Pydantic model with `extra: "allow"` config — validates known fields (`page_size` max 20, `orientation` portrait|landscape, `watermark` max 100, `header`/`footer` max 500, `margin_mm` 0-100, `quality` draft|standard|high) while allowing unknown fields for forward compatibility; replaced `dict` with `Optional[ExportOptions]` on all 8 format endpoints (4) ~~Bulk export has no limit on `document_ids` count~~ **FIXED**: Added `max_length=100` on `BulkExportRequest.document_ids`; added `max_length=50` on `EmailCampaignRequest.document_ids` and `max_length=500` on `EmailCampaignRequest.recipients`; added `subject` max 500, `message` max 10,000 (5) Email campaign sends sequentially — N docs × M recipients = N×M API calls (6) ~~Portal publish accepts password in plaintext request body~~ **FIXED**: Portal publish response now strips password from result dict and nested options dict; adds `password_protected: bool` indicator instead of echoing password. **ADDED (Security Hardening)**: Per-endpoint rate limiting — `@limiter.limit(RATE_LIMIT_STRICT)` on `bulk_export`; `@limiter.limit(RATE_LIMIT_STANDARD)` on individual format exports (pdf, docx, pptx, teams).
...
- **ADDED (Security Hardening — Rate Limiting)**: Per-endpoint rate limit tier constants: `RATE_LIMIT_STRICT = "10/minute"` (AI, color generation, ingestion), `RATE_LIMIT_STANDARD = "60/minute"` (mutations, exports). Applied via `@limiter.limit()` decorators across ai.py (12 endpoints), design.py (3), ingestion.py (4), export.py (5), documents.py (3). Total: ~27 endpoints with per-endpoint limits. **FIXED (Decorator Order)**: All 27 occurrences had incorrect decorator order (`@limiter.limit` above `@router.post`); swapped to correct order (`@router.post` outer, `@limiter.limit` inner) across 5 files (ai.py, design.py, documents.py, export.py, ingestion.py) so FastAPI registers the limiter-wrapped function. Tests: `test_rate_limiting.py` (6 tests, 1 passing — slowapi v0.1.9 `_auto_check` mode has TestClient integration limitation). Global autouse fixture added to conftest.py to disable limiter for all tests except rate_limiting tests.
