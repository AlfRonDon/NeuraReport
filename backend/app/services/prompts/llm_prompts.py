...
LLM_CALL_4_SYSTEM_PROMPT = dedent(
    ...
    * Describe reshape rules with exact column ordering, filters to apply before aggregation, and any dedup or ordering requirements. Ensure `step5_requirements.datasets.rows.grouping` / `ordering` mirror those expectations.
      ...
      "step5_requirements": {
        "datasets": {
          ...
          "rows": {
            ...
            "ordering": ["Describe stable sort order with tie-breakers and case handling (e.g., ORDER BY material_name ASC, bin_index ASC)."]
          },
