...
import ConnectDBContainer from './ConnectDBContainer'
import UploadVerifyContainer from './UploadVerifyContainer'
import TemplatesPaneContainer from './TemplatesPaneContainer'
...
export default function SetupPage() {
  ...
  const executeUI = (label, action, intent = {}) => {
    return execute({
      ...
      intent: { source: 'setup', ...intent },
      action,
    })
  }

  const handleSetupNavChange = (next) => {
    if (!next) return
    executeUI('Switch setup step', () => setSetupNav(next), { step: next })
  }

  const handleToggleTips = () => {
    executeUI('Toggle setup tips', () => setDetailsOpen((prev) => !prev))
  }

  const handleGoToConnect = () => {
    executeUI('Open connect step', () => setSetupNav('connect'))
  }
  ...
  return (
    <Box sx={{ py: 3, px: 3 }}>
      ...
      <Stack spacing={3}>
        ...
        <Surface sx={{ p: 2 }}>
          ...
          >
            <ToggleButtonGroup
              value={setupNav}
              exclusive
              onChange={(e, val) => handleSetupNavChange(val)}
        ...
        {/* Tab Content */}
        {setupNav === 'connect' && <ConnectDBContainer />}

        {setupNav === 'generate' && <UploadVerifyContainer />}

        {setupNav === 'templates' && (
          <TemplatesPaneContainer
            apiBase={apiBase}
