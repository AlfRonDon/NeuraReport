...
import { MainPanel, WorkspaceHeader } from '../../../components/shell'
...
export default function AnalyzePageContainer() {
  ...
  // Build status for WorkspaceHeader
  const headerStatus = analysisResult
    ? { label: `${analysisResult.tables?.length || 0} tables found`, color: 'success', variant: 'outlined' }
    : selectedFile
    ? { label: selectedFile.name, color: 'primary', variant: 'outlined' }
    : null
      ...
      <Stack spacing={3}>
        ...
        <Box>
          ...
          <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
            ...
            {statusChips.length > 0 && statusChips.map((chip, idx) => (
              <Chip
                key={idx}
                label={chip.label}
                size="small"
